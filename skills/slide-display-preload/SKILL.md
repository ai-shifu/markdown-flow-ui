...
- 当 sandbox vendor 体积较大时，不要在模块顶层做全局预热；应只在 `type === "sandbox"` 的真实渲染分支按需懒加载，并在该分支记录加载耗时日志用于性能回归对比。
- 当 `ContentRender` 普通 markdown 路径不需要 iframe 时，不要同步引入 `IframeSandbox`；优先使用按需动态加载，并在进入 sandbox 前加快速标签短路判断，避免每次普通渲染都触发重分段逻辑。
