...
- 当 `Slide` 需要兜底处理“流式 SSE 长时间无新消息”时，优先在组件层新增显式 `isStreaming + streamInactivityTimeoutMs + onError` 能力，并通过统一错误码向上抛出；不要在 `audio segment` 层猜测历史记录还是流。
- 当 `Slide.onError` 需要让业务侧参与清理播放中间态时，错误上下文应显式暴露 buffering loading 的开关函数，让外层在 toast、模式切换或停流收尾时可以再次关闭 loading，而不是只能依赖组件内部副作用。
