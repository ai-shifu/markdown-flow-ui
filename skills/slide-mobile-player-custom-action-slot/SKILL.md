...
30a. 若产品明确不需要移动端进入 `fullscreen` 后的旋转/体验提示，应同时移除提示组件渲染与进入 fullscreen 时的唤起状态逻辑，不要只清空文案或把时长改成 `0`，避免残留遮罩闪现。
...
33. 若产品要求 `Slide` 在移动端无论横屏还是竖屏都以“非全屏”作为默认初始态，不要再把设备物理方向或宽高比自动映射成 `fullscreen`；应仅在用户显式切换后进入全屏，并让设置面板默认选中 `nonFullscreen`。
34. 若移动端 settings sheet 同时存在“标题下分隔线”和“设置项下分隔线”，标题下方的首条分隔线默认保持整行全宽；只有具体设置项之间的分隔线再做左右 `24px` 内缩，不要把标题下方也误做成 inset divider。
35. 若移动端 settings sheet 设计要求更紧凑的留白，优先把标题、设置项与 inset divider 的左右内边距统一收敛到 `16px`；像字幕开关这类右侧控制，默认使用 `24x24` 的点击区域，再在里面放实际的开关图形，避免视觉尺寸和点击热区混在一起。
