...
- 当当前步骤先播完已有 `audio_segments`、随后后端再补回完整 `audio_url` 时，若播放器恢复 seek 的位置已经达到或超过该 URL 的实际时长，应直接把该步骤视为“已播放完成”并触发自动前进；不要只关闭 loading 后停在当前页等待不会再来的 `ended` 事件。
...
- 当多选选项文案自身包含英文逗号、中文逗号或换行时，不要先按分隔符粗暴 `split` 历史 `user_input`；应优先基于当前交互块的已知选项做“最长前缀匹配/贪心消费”，只把最终未匹配的尾部文本视为自定义输入，避免把真实选项误塞进输入框。
