...
- 当纯流式播放正停在“最后一个静默 marker step”（例如无音频的 `html` 首屏）上，而 SSE append 进来新的 marker 时，也应立即切到这个新 marker；不要继续沿用旧 step 的 `markerAutoAdvanceDelay`，否则即使新 element 已到，仍会额外等待 2 秒左右才翻页。
